...
def configure_interface():
    ...
    if uploaded_file is not None:
        ...
        if blob_url:
            st.write(f"Arquivo {fileName} enviando com sucesso para o Azure Blob Storage")
            credit_card_info= analize_credit_card (blob_url) # Chamar a funão de detecção de informações de cartão de crédito
...
def show_image_and_validation (blob_url, credit_card_info ):
    ...
    if credit_card_info and credit_card_info["card_name"]:
        ...
        st.markdown(f"<h1 style= 'color: red;' > Cratão Inválido</h1>")
